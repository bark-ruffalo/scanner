...
code2prompt -p src -p templates -p render.yaml -p middleware.ts -p instrumentation.ts -p package.json -p biome.jsonc -p drizzle.config.ts -p next.config.js -p postcss.config.js -p tsconfig.json -p .env -p .env.example -o context-codebase.md
